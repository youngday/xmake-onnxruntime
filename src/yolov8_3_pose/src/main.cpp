...
std::vector<int> limbColorIndices = {9, 9, 9, 9, 7, 7, 7, 0, 0, 0,
                                     0, 0, 16, 16, 16, 16, 16, 16, 16};
std::vector<int> kptColorIndices = {16, 16, 16, 16, 16, 0, 0, 0, 0,
                                    0, 0, 9, 9, 9, 9, 9, 9};
...
cv::Scalar generateRandomColor(int numChannels)
{
  if (numChannels < 1 || numChannels > 3)
  {
    throw std::invalid_argument(
        "Invalid number of channels. Must be between 1 and 3.");
  }
...
std::vector<cv::Scalar> generateRandomColors(int class_names_num,
                                             int numChannels)
{
...
void plot_masks(cv::Mat img, std::vector<YoloResults> &result,
                ...
                std::unordered_map<int, std::string> &names)
{
  ...
  for (int i = 0; i < result.size(); i++)
  {
    ...
    putText(img, label, cv::Point(left - 1.5, top - 2.5),
                  ...
                  const cv::Size &shape)
{
  ...
  for (const auto &res : results)
  {
    ...
    if (!res.keypoints.empty())
    {
      ...
      bool isPose = keypoint.size() ==
                    51; // numKeypoints == 17 && keypoints[0].size() == 3;
      drawLines &= isPose;
      ...
      for (int i = 0; i < 17; i++)
      {
        ...
        if (x_coord % raw_image_shape.width != 0 &&
            y_coord % raw_image_shape.height != 0)
        {
          if (keypoint.size() == 3)
          {
          ...
          cv::Scalar color_k =
              isPose ? kptColorPalette[i]
                     : cv::Scalar(0, 0,
                                  255); // Default to red if not in pose mode
          cv::circle(img, cv::Point(x_coord, y_coord), radius, color_k, -1,
                     cv::LINE_AA);
        }
      }
      // draw lines
      if (drawLines)
      {
        for (int i = 0; i < skeleton.size(); i++)
        {
          ...
          if (x1 % raw_image_shape.width == 0 ||
              y1 % raw_image_shape.height == 0 || x1 < 0 || y1 < 0 ||
              x2 % raw_image_shape.width == 0 ||
              y2 % raw_image_shape.height == 0 || x2 < 0 || y2 < 0)
          {
            continue;
          }
          ...
          cv::line(img, cv::Point(x1, y1), cv::Point(x2, y2), color_limb, 2,
                   cv::LINE_AA);
        }
      }
    }
...
int main()
{
  ...
  float mask_threshold = 0.5f; // in python it's 0.5 and you can see that at
                               // ultralytics/utils/ops.process_mask line 705
                               // (ultralytics.__version__ == .160)
