...
AutoBackendOnnx::AutoBackendOnnx(
    ...
    const std::vector<int> &imgsz, const int &stride, const int &nc,
    const std::unordered_map<int, std::string> names)
    : OnnxModelBase(modelPath, logid, provider), imgsz_(imgsz), stride_(stride),
      nc_(nc), names_(names), inputTensorShape_() {}
...
AutoBackendOnnx::AutoBackendOnnx(const char *modelPath, const char *logid,
  ...
  //  ideally you should get all of them but you'll raise error if smth is not
  //  in metadata (or not under the appropriate keys)
  const std::unordered_map<std::string, std::string> &base_metadata =
      OnnxModelBase::getMetadata();
...
std::vector<YoloResults> AutoBackendOnnx::predict_once(cv::Mat &image,
  ...
  letterbox(image, preprocessed_img, new_shape, cv::Scalar(), auto_, scaleFill,
            true, getStride());
  ...
  if (task_ == YoloTasks::SEGMENT) {
    ...
    cv::Mat output0 =
        cv::Mat(
            cv::Size((int)outputTensor0Shape[2], (int)outputTensor0Shape[1]),
            CV_32F, all_data0)
    ...
    std::vector<int> mask_sz = {1, (int)mask_shape[1], (int)mask_shape[2],
                                (int)mask_shape[3]};
    cv::Mat output1 = cv::Mat(mask_sz, CV_32F,
                              outputTensors[1].GetTensorMutableData<float>());
  ...
  LOG_INFO(logger, "image size h*w:{0}*{1}", preprocessed_img.rows,
  ...
  LOG_INFO(logger, "shape (1, {0},{1},{2})", image.channels(),
           preprocessed_img.rows, preprocessed_img.cols);

  return results;
}

void AutoBackendOnnx::postprocess_masks(
  ...
  for (int r = 0; r < rows; ++r) {
    ...
    if (max_conf > conf_threshold) {
      ...
      cv::Rect_<float> scaled_bbox =
          scale_boxes(getCvSize(), bbox, image_info.raw_size);
  ...
  for (int i = 0; i < nms_result.size(); ++i) {
    ...
    _get_mask2(cv::Mat(masks[idx]).t(), proto, image_info, boxes[idx],
               result.mask, mask_threshold, iw, ih, mw, mh, masks_features_num);
...
void AutoBackendOnnx::postprocess_detects(
  ...
  for (int r = 0; r < rows; ++r) {
    ...
    if (max_conf > conf_threshold) {
      ...
      cv::Rect_<float> bbox =
          cv::Rect_<float>(out_left, out_top, (out_w + 0.5), (out_h + 0.5));
      cv::Rect_<float> scaled_bbox =
          scale_boxes(getCvSize(), bbox, image_info.raw_size);
...
void AutoBackendOnnx::postprocess_kpts(cv::Mat &output0, ImageInfo &image_info,
  ...
  std::tie(boxes, confidences, class_ids, rest) = non_max_suppression(
      output0, class_names_num, output0.cols, conf_threshold, iou_threshold);
  ...
  for (int i = 0; i < boxes.size(); i++) {
    ...
    //            if len(pred) else pred[:, 6:] pred_kpts =
    //            ops.scale_coords(img.shape[2:], pred_kpts, shape) path =
    //            self.batch[0] img_path = path[i] if isinstance(path, list)
    //            else path results.append(
    ...
    std::vector<float> kpt =
        scale_coords(img1_shape, rest[i], image_info.raw_size);
...
void AutoBackendOnnx::_get_mask2(const cv::Mat &masks_features,
                                 ...
                                 const cv::Rect bound, cv::Mat &mask_out,
                                 float &mask_thresh, int &iw, int &ih, int &mw,
                                 int &mh, int &masks_features_num,
...

{
  ...
  cv::Rect_<float> downsampled_bbox =
      scale_boxes(img0_shape, bound_float, downsampled_size);
  ...
  cv::Mat resized_mask;
  cv::Rect_<float> input_bbox =
      scale_boxes(img0_shape, bound_float, img1_shape);
